...
const QCChartApp = () => {
    ...
    const handleCanvasMouseUp = () => {
        if (dragArrowStart && tempArrow) {
            const newArrow = {
                ...
                points: [tempArrow.start, tempArrow.end],
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            pdf.save(`${metaData.brand}_QC_${exportDPI}dpi.pdf`);
        } catch (e) {
            console.error(e);
            alert("導出失敗，請重試");
        } finally {
    ...
    const generatePPT = async () => {
        ...
        try {
            ...
            pptx.writeFile({ fileName: `${metaData.brand}_QC_${exportDPI}dpi.pptx` });
        } catch (e) {
            console.error(e);
        } finally {
            setActivePageIndex(originalIndex);
    ...
    const currentLibraryItems = libraryAssets.filter(item => {
        const matchesFolder = item.folder === activeFolder || (!item.folder && activeFolder === 'TM');
        ...
        <div className="flex flex-col h-screen bg-gray-100 font-sans overflow-hidden">
            ...
            <div className="bg-slate-800 text-white p-2 shadow-md z-30 flex flex-col gap-2 shrink-0">
                <div className="flex justify-between items-center px-2">
                    <h1 className="text-lg font-bold flex items-center gap-2">
                        ...
                        QC Visualizer <span className="text-xs text-gray-400">v4.0 Pro</span>
                    </h1>
                    ...
                    <div className="flex gap-2">
                        <button onClick={saveProject} className="flex items-center gap-2 px-3 py-1 bg-green-600 hover:bg-green-500 rounded text-sm font-semibold"><Save size={16} /> 儲存專案</button>
                        <button onClick={() => setShowHistory(true)} className="flex items-center gap-2 px-3 py-1 bg-slate-600 hover:bg-slate-500 rounded text-sm"><History size={16} /> 歷史</button>
                        <button onClick={() => setShowExportOptions(true)} className="flex items-center gap-2 px-3 py-1 bg-blue-600 hover:bg-blue-500 rounded text-sm font-semibold"><Download size={16} /> 導出</button>
                    </div>
                </div>
            ...
            {isLoading && (
                <div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center text-white flex-col">
                    ...
                    <div>處理中... (正在生成 {exportDPI} DPI 高解析文件)</div>
                </div>
            )}

            {/* Export Options Modal */}
            {showExportOptions && (
                <div className="absolute inset-0 bg-black/50 z-40 flex items-center justify-center" onClick={() => setShowExportOptions(false)}>
                    <div className="bg-white rounded-lg p-6 w-96 shadow-2xl" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><Settings size={20} /> 導出設定</h3>

                        <div className="mb-4">
                            <label className="block text-sm font-medium mb-2">圖片品質 (DPI)</label>
                            <div className="flex gap-2">
                                {[96, 150, 300].map(dpi => (
                                    <button
                                        key={dpi}
                                        onClick={() => setExportDPI(dpi)}
                                        className={`flex-1 py-2 rounded border-2 font-semibold transition-colors ${exportDPI === dpi ? 'bg-blue-500 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300 hover:border-blue-400'}`}
                                    >
                                        {dpi} DPI
                                        <div className="text-xs font-normal">{dpi === 96 ? '網頁' : dpi === 150 ? '標準' : '高品質'}</div>
                                    </button>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                            <button onClick={generatePDF} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded font-semibold">
                                <Download size={18} /> PDF
                            </button>
                            <button onClick={generatePPT} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-orange-600 hover:bg-orange-500 text-white rounded font-semibold">
                                <MonitorPlay size={18} /> PPT
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/* History Modal */}
            {showHistory && (
                <div className="absolute inset-0 bg-black/50 z-40 flex items-center justify-center" onClick={() => setShowHistory(false)}>
                    <div className="bg-white rounded-lg p-6 w-[600px] max-h-[80vh] overflow-auto shadow-2xl" onClick={e => e.stopPropagation()}>
                        <h3 className="text-lg font-bold mb-4 flex items-center gap-2"><History size={20} /> 專案歷史</h3>

                        {savedVersions.length === 0 ? (
                            <div className="text-center text-gray-400 py-10">尚無儲存的專案</div>
                        ) : (
                            <div className="space-y-2">
                                {savedVersions.map(project => (
                                    <div key={project.id} className="flex items-center justify-between p-3 border rounded hover:bg-gray-50">
                                        <div className="flex-1">
                                            <div className="font-semibold">{project.name}</div>
                                            <div className="text-xs text-gray-500">
                                                {new Date(project.savedAt).toLocaleString('zh-TW')} · {project.pages.length} 頁
                                            </div>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => loadProject(project)} className="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600">載入</button>
                                            <button onClick={() => deleteProject(project.id)} className="px-3 py-1 bg-red-500 text-white rounded text-sm hover:bg-red-600">刪除</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="w-14 bg-white border-r border-gray-200 flex flex-col items-center py-4 gap-4 z-10 shadow-sm shrink-0">
                    ...
                    <ToolButton active={mode === 'arrow'} onClick={() => { setMode('arrow'); setSelectedId(null); setShowArrowSettings(true); }} icon={<ArrowRight size={20} />} label="箭頭" />
                ...
                {showArrowSettings && (
                    <div className="absolute left-16 top-4 bg-white border border-gray-300 rounded-lg shadow-xl p-4 z-20 w-64">
                        <div className="flex justify-between items-center mb-3">
                            <h4 className="font-bold text-sm flex items-center gap-2"><Palette size={16} /> 箭頭設定</h4>
                            <button onClick={() => setShowArrowSettings(false)} className="text-gray-400 hover:text-gray-600"><X size={16} /></button>
                        </div>

                        <div className="space-y-3">
                            <div>
                                <label className="text-xs font-medium block mb-1">粗細</label>
                                <div className="flex items-center gap-2">
                                    <input
                                        ...
                                        max="8"
                                        step="0.5"
                                        value={arrowSettings.width}
                                        onChange={e => setArrowSettings({ ...arrowSettings, width: parseFloat(e.target.value) })}
                                        className="flex-1"
                                    />
                                    <span className="text-xs w-8 text-right">{arrowSettings.width}px</span>
                                </div>
                            </div>

                            <div>
                                <label className="text-xs font-medium block mb-1">顏色</label>
                                <div className="flex gap-2">
                                    {['#dc2626', '#ea580c', '#ca8a04', '#16a34a', '#2563eb', '#9333ea', '#000000'].map(color => (
                                        <button
                                            key={color}
                                            onClick={() => setArrowSettings({ ...arrowSettings, color })}
                                            className={`w-8 h-8 rounded border-2 ${arrowSettings.color === color ? 'border-gray-800 scale-110' : 'border-gray-300'}`}
                                ...
                                <input
                                    ...
                                    onChange={e => setArrowSettings({ ...arrowSettings, color: e.target.value })}
                                    className="w-full mt-2 h-8 rounded cursor-pointer"
                                />
                            </div>

                            <div>
                                <label className="text-xs font-medium block mb-1">樣式</label>
                                <div className="flex gap-2">
                                    <button
                                        onClick={() => setArrowSettings({ ...arrowSettings, style: 'solid' })}
                                        className={`flex-1 py-2 text-xs rounded border ${arrowSettings.style === 'solid' ? 'bg-blue-500 text-white border-blue-600' : 'bg-white border-gray-300'}`}
                                    >
                                        實線
                                    </button>
                                    <button
                                        onClick={() => setArrowSettings({ ...arrowSettings, style: 'dashed' })}
                                        className={`flex-1 py-2 text-xs rounded border ${arrowSettings.style === 'dashed' ? 'bg-blue-500 text-white border-blue-600' : 'bg-white border-gray-300'}`}
                                    >
                                        虛線
                                    </button>
                                </div>
                                ...
                                <div className="relative w-full h-[calc(100%-5rem)] bg-white overflow-hidden">
                                    ...
                                    <svg className={`absolute inset-0 w-full h-full z-20 pointer-events-none ${!activePage.isMainImageLocked ? 'opacity-50' : ''}`}>
                                        <defs>
                                            ...
                                            {['#dc2626', '#ea580c', '#ca8a04', '#16a34a', '#2563eb', '#9333ea', '#000000'].map(color => (
                                                <marker key={color} id={`arrowhead-${color.replace('#', '')}`} markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
                                        ...
                                        {activePage.arrows.map(arrow => (
                                            <EditableArrow
                                                ...
                                                isEditing={!activePage.isMainImageLocked}
                                            />
                                        ))}
                                        ...
                                        {tempArrow && (
                                            <line
                                                ...
                                                strokeWidth={arrowSettings.width}
                                                strokeDasharray={arrowSettings.style === 'dashed' ? '4' : '0'}
                                                markerEnd={`url(#arrowhead-${arrowSettings.color.replace('#', '')})`}
                                            />
                                        )}
                                    </svg>
...
// --- Enhanced Arrow Component with Polyline Support ---
const EditableArrow = ({ arrow, isSelected, onSelect, onUpdate, isEditing }) => {
    const points = arrow.points || [arrow.start, arrow.end];
    const color = arrow.color || '#dc2626';
    ...
    return (
        <g onClick={onSelect} className={`${isSelected ? 'opacity-100' : 'opacity-80'} hover:opacity-100 cursor-pointer pointer-events-auto`}>
            ...
            <path
                ...
                strokeWidth={width}
                fill="none"
                strokeDasharray={style === 'dashed' ? '8 4' : '0'}
                markerEnd={`url(#arrowhead-${color.replace('#', '')})`}
            />

            {isSelected && isEditing && points.map((pt, i) => (
                <circle
                    key={i}
                    cx={pt.x}
                    cy={pt.y}
